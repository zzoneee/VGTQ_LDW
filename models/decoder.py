...
class DeepFusion(nn.Module):
    def __init__(self, d_model=256, num_heads=8, attn_drop=0.0, qkv_bias=False):
        super().__init__()
        ...
        self.img_qv = nn.Linear(d_model, d_model * 2, bias=qkv_bias)
        self.word_qv = nn.Linear(d_model, d_model * 2, bias=qkv_bias)

        self.img_proj = nn.Linear(d_model, d_model)
        self.word_proj = nn.Linear(d_model, d_model)

        self.img_norm1 = nn.LayerNorm(d_model)
        self.word_norm1 = nn.LayerNorm(d_model)

        self.img_mlp = Mlp(in_features=d_model, hidden_features=d_model * 4, drop=0.0)
        self.word_mlp = Mlp(in_features=d_model, hidden_features=d_model * 4, drop=0.0)

        self.img_norm2 = nn.LayerNorm(d_model)
        self.word_norm2 = nn.LayerNorm(d_model)
    ...
    def forward(self, img_feat, img_key_padding_mask, img_pos,
        ...
        word_feat = self.word_norm1(word_feat)

        N, B, C = img_feat.shape
        img_feat = img_feat.permute(1, 0, 2)  # B, N, C
        img_qv = self.img_qv(img_feat)
        img_q, img_v = img_qv.view(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).unbind(0)

        N_word, B, C = word_feat.shape
        word_feat = word_feat.permute(1, 0, 2)  # B, N, C
        word_qv = self.word_qv(word_feat)
        word_q, word_v = word_qv.view(B, N_word, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).unbind(0)

        B, num_heads, N, _ = img_q.shape
        img_q = img_q * self.scale
        attn = (img_q @ word_q.transpose(-2, -1))

        attn_img_word = attn.softmax(dim=-1)
        attn_img_word = self.attn_drop(attn_img_word)
        fuse_img_feat = (attn_img_word @ word_v).transpose(1, 2).reshape(B, N, -1).permute(1, 0, 2)

        attn_word_img = attn.transpose(-2, -1).softmax(dim=-1)
        attn_word_img = self.attn_drop(attn_word_img)
        fuse_word_feat = (attn_word_img @ img_v).transpose(1, 2).reshape(B, N_word, -1).permute(1, 0, 2)

        # return torch.cat([orig_img_feat, fuse_img_feat], dim=-1), torch.cat([orig_word_feat, fuse_word_feat], dim=-1)
        fuse_img_feat = orig_img_feat + self.img_proj(fuse_img_feat)
        fuse_word_feat = orig_word_feat + self.word_proj(fuse_word_feat)

        fuse_img_feat = fuse_img_feat + self.img_mlp(self.img_norm2(fuse_img_feat))
        fuse_word_feat = fuse_word_feat + self.word_mlp(self.word_norm2(fuse_word_feat))

        return fuse_img_feat, fuse_word_feat


class NewMultiStageDecoderLayer(nn.Module):
    ...
    def forward(self, vis_query, vis_query_pos, text_query_pos,
                ...
                img_feat_k=None, word_feat_k=None):
        ...
        # img_feat_k = 0.0 * img_feat_k + 1.0 * img_feat * self.sigmoid(
        #     (img_feat.transpose(0, 1) @ vis_query.permute(1, 2, 0)).transpose(0, 1))
        # word_feat_k = 0.0 * word_feat_k + 1.0 * word_feat * self.sigmoid(
        #     (word_feat.transpose(0, 1) @ vis_query.permute(1, 2, 0)).transpose(0, 1))

        img_feat_k = img_feat * self.sigmoid(
            (img_feat.transpose(0, 1) @ vis_query.permute(1, 2, 0)).transpose(0, 1))
        word_feat_k = word_feat * self.sigmoid(
            (word_feat.transpose(0, 1) @ vis_query.permute(1, 2, 0)).transpose(0, 1))

        return vis_query, img_feat_k, word_feat_k
...
class DecoderWithExtraEncoder(nn.Module):
    def __init__(self, num_queries, query_dim,
        ...
        self.word_norm = nn.LayerNorm(256)
    ...
    def forward(self, img_feat, img_key_padding_mask=None, pos=None,
        ...
        # print("*********************img_feat*********************:", len(img_feat), img_feat[0].shape, img_feat)
        # 5, torch.Size([25600, 4, 256])[tensor([[[0.4744, -0.1434, -0.1672, ..., 2.2938, -0.3444, 0.3868],

        # print("*********************img_feat*********************:", img_feat[3].shape)
        # torch.Size([400, 4, 256])

        img_feat = img_feat[3]
        hw, bs, c = img_feat.shape

        # print("*********************img_feat*********************:", len(img_key_padding_mask), img_key_padding_mask[0].shape, img_key_padding_mask[3].shape, img_key_padding_mask)
        # 5 torch.Size([4, 25600]) torch.Size([4, 400]) [tensor([[False, False, False,  ..., False, False, False],

        img_key_padding_mask = img_key_padding_mask[3]

        # Encode discriminative features
        for layer in self.extra_encoder_layers:
            img_feat, word_feat = layer(img_feat, img_key_padding_mask, pos,
        ...
        # Language-guided Target Query Module

        # *********************v1 start*******************
        word_feat_one = (word_feat.sum(0) / word_feat.shape[0]).unsqueeze(1)  # B, 1, C
        img_attn = (self.img_norm(self.img_proj(img_feat_srcs[0].transpose(0, 1))) @ self.word_norm(
            self.word_proj(word_feat_one)).transpose(1, 2)).squeeze(-1)  # B, WH
        img_attn = img_attn.softmax(dim=-1)  # B, WH
        img_attn = img_attn.unsqueeze(-1)  # B, WH, 1
        vis_query = img_feat_srcs[0].transpose(0, 1) * img_attn  # B, WH, C
        vis_query = (vis_query.sum(1) / vis_query.shape[1]).unsqueeze(0)  # 1, B, C
        # *********************v1 end*******************
        ...
        # img_attn = (self.img_norm(self.img_proj(img_feat_srcs[0].transpose(0, 1))) @ self.word_norm(
        #     self.word_proj(word_feat_one)).transpose(1, 2)).squeeze(-1)  # B, WH
        # img_attn = (img_attn / 16.0).softmax(dim=-1)  # B, WH
        # img_attn = img_attn.unsqueeze(-1)  # B, WH, 1
        # vis_query = img_feat_srcs[0].transpose(0, 1) * img_attn  # B, WH, C
        # vis_query = (vis_query.sum(1)).unsqueeze(0)  # 1, B, C
        # *********************v2 end*******************
        ...
        # Multi-stage decoder
        # img_feat_k = img_feat
        word_feat_k = word_feat

        img_feat_srcs = img_feat.chunk(2, dim=-1)
        img_feat_k = img_feat_srcs[1]
        img_feat = img_feat_srcs[0]

        for idx, layer in enumerate(self.layers):
            vis_query, img_feat_k, word_feat_k = layer(vis_query, vis_query_embed, text_query_embed,
                              img_feat, img_key_padding_mask, pos,
                              word_feat, word_key_padding_mask, None, idx, img_feat_k, word_feat_k)
...
class DiscriminativeFeatEncLayer(nn.Module):
    def __init__(self, d_model, img2text_attn_args=None, img_query_with_pos=True,
                 ...
                 num_heads=8, attn_drop=0.0, qkv_bias=False):
        super().__init__()
    ...
    def forward(self, img_feat, img_key_padding_mask, img_pos,
        ...
        word_feat = word_feat.permute(1, 0, 2)  # B, N, C
        word_qv = self.word_qv(word_feat)
        word_q, word_v = word_qv.view(B, N_word, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).unbind(
            0)
        ...
        img_feat = fuse_img_feat + self.img_mlp(self.img_norm2(fuse_img_feat))
        word_feat = fuse_word_feat + self.word_mlp(self.word_norm2(fuse_word_feat))
        ...
        return torch.cat([orig_img_feat, fuse_img_feat], dim=-1), word_feat
...
_MODULES = {
    ...
    'DeepFusion': DeepFusion,
    'NewMultiStageDecoderLayer': NewMultiStageDecoderLayer,
}
