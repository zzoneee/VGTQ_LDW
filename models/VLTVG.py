...
class VLTVG(nn.Module):
    def __init__(self, pretrained_weights, args=None):
        ...
        # Image feature encoder (Swin Transformer encoder)
        ...
        cfg.model.backbone.use_spatial = False
        cfg.model.backbone.use_channel = False
        cfg.model.neck.use_spatial = False
        cfg.model.neck.use_channel = False
    ...
    def forward(self, image, image_mask, word_id, word_mask):
        ...
        # Image features （swin-s）
        import torch.nn.functional as F
        """Directly extract features from the backbone+neck."""
        img_feat = self.backbone(image, word_feat)

        img_feat = self.neck(img_feat, word_feat)
        mask = [F.interpolate(image_mask[None].float(), size=_.shape[-2:]).to(torch.bool)[0] for _ in img_feat]

        # flatten
        shape = [_.shape[-2:] for _ in img_feat]
        img_feat = [rearrange(_, 'B C H W -> (H W) B C') for _ in img_feat]
        mask = [rearrange(_, 'B H W -> B (H W)') for _ in mask]
        # print("*********************x.shape, out_mask.shape*********************:", len(x), len(out_mask), x[0].shape,
        #       out_mask[0].shape)  # 5 5 torch.Size([25600, 16, 256]) torch.Size([16, 25600])
        # return x, out_mask
