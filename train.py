...
def get_args_parser():
    ...
    parser.add_argument('--data_root', default='./ln_data/')
...
def main(args):
    ...
    # backbone_param = [p for n, p in model_without_ddp.backbone.named_parameters() if p.requires_grad and
    #                   (not n.endswith('para_norm.weight')) and (not n.endswith('para_norm.bias')) and
    #                   (not n.endswith('para1_1')) and (not n.endswith('para1_2'))
    #                   and (not n.endswith('para0')) and (not n.endswith('KV_text.weight'))
    #                   and (not n.endswith('Q_text.weight')) and (not n.endswith('proj_text.weight'))
    #                   and (not n.endswith('KV_text.bias'))
    #                   and (not n.endswith('Q_text.bias')) and (not n.endswith('proj_text.bias'))]

    backbone_param = [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and
                      (n.startswith('backbone') or n.startswith('neck')) and
                      (not (n.find('attTextP') != -1))]
    ...
    #               (n.startswith('trans_decoder') or n.startswith('bbox_embed') or n.startswith('bert_proj') or
    #                n.endswith('para_norm.weight') or n.endswith('para_norm.bias') or n.endswith('para1_1') or
    #                n.endswith('para1_2') or n.endswith('para0') or n.endswith('KV_text.weight')
    #                or n.endswith('Q_text.weight') or n.endswith('proj_text.weight') or n.endswith('KV_text.bias')
    #                or n.endswith('Q_text.bias') or n.endswith('proj_text.bias'))]
    rest_param = [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and
                  (n.startswith('trans_decoder') or n.startswith('bbox_embed') or n.startswith('bert_proj')
                   or n.find('attTextP') != -1)]
    ...
    logger.info(f'The num of learnable parameters: backbone({cnt_backbone}), '
                f'bert({cnt_bert}), rest({cnt_rest})')
    logger.info(f'Check the whole parameters: {cnt_whole} = {cnt_backbone + cnt_bert + cnt_rest}')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            for checkpoint_path in checkpoint_paths:
                ...
                # oss_path = "oss://pai-vision-data-hz/data/demo/ChatGLM/QRNet/VGTQ/log/unc/VGTQ/"
                oss_path = "oss://pai-vision-data-hz/data/demo/ChatGLM/QRNet/VGTQ/swin-s/neck/dynamic_mlpAndMixmlp_duli_attTextShareNoMlp_lr/flick/log/"
                out_path = str(checkpoint_path)
                cmd = f"ossutil cp -u {out_path} {oss_path}"
                os.system(cmd)
        ...
        if args.output_dir and utils.is_main_process():
            ...
            # oss_path = "oss://pai-vision-data-hz/data/demo/ChatGLM/QRNet/VGTQ/log/unc/VGTQ/" + str(timestap) + "/"
            oss_path = "oss://pai-vision-data-hz/data/demo/ChatGLM/QRNet/VGTQ/swin-s/neck/dynamic_mlpAndMixmlp_duli_attTextShareNoMlp_lr/flick/log/" + str(timestap) + "/"
            out_path = args.output_dir + '/iter.log'
            cmd = f"ossutil cp -u {out_path} {oss_path}"
            os.system(cmd)

            out_path = args.output_dir + '/epoch.log'
            cmd = f"ossutil cp -u {out_path} {oss_path}"
            os.system(cmd)
